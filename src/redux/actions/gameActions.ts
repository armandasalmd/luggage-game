...
import { addCoins } from "./userActions";
...
export const gameEnded = (state: IGameRewards, myUsername: string) => {
  return function (dispatch: Dispatch) {
    dispatch({ type: ActionTypes.GameEnded, payload: state });
    
    const myReward = [state.looser, ...state.winners].find(o => o.username === myUsername);
    if (myReward) addCoins(myReward.reward)(dispatch);
  }
}
