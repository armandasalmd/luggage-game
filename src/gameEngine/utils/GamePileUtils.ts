...
export function throwFrom(playerIndex: number): ISpringTransform {
  const playerRect = document.querySelector(".player" + playerIndex)!.getBoundingClientRect();
  const dropzoneRect = document.querySelector(".playground__targetDropzone")!.getBoundingClientRect();

  return {
    x: (-dropzoneRect.x + playerRect.x) / 0.9 - playerRect.width / 3,
...
export function toPileCardElement(cardElement: HTMLElement): HTMLElement | null {
